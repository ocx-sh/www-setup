...
# parse / checksum failure and NEVER prints a partial manifest. Generation is
# retried as a whole (see below) and, once exhausted, this script exits 3 BEFORE
# any upload, so a transient GitHub outage during the hourly cron cannot
# overwrite the live dist.json. We stage the generated bytes to a mktemp file
# first and only upload that file.
#
...
# unparseable body, sha256.sum fetch failure) gen-dist.sh exits 3 and NEVER
# prints a partial manifest — the live dist.json is left untouched.
STAGE=$(mktemp -d)
trap 'rm -rf "$STAGE"' EXIT INT TERM
DIST_TMP="$STAGE/dist.json"

# GitHub serves release assets through a CDN that intermittently 500s a healthy
# sha256.sum for minutes at a time (both the /releases/download URL and the API
# asset endpoint redirect to the same blob, so there is no second door). The
# generator's per-request curl retries cover a blip of seconds; a whole-run
# retry covers a longer one. Re-generating is a pure read of the Releases API,
# so repeating it is free of side effects.
GEN_ATTEMPTS="${GEN_ATTEMPTS:-3}"
GEN_RETRY_DELAY="${GEN_RETRY_DELAY:-60}"
gen_attempt=1
while :; do
    if sh "$GEN_DIST" >"$DIST_TMP"; then
        break
    fi
    if [ "$gen_attempt" -ge "$GEN_ATTEMPTS" ]; then
        echo "publish-dist: manifest generation failed after ${GEN_ATTEMPTS} attempts" >&2
        exit 3
    fi
    echo "publish-dist: manifest generation failed (attempt ${gen_attempt}/${GEN_ATTEMPTS}), retrying in ${GEN_RETRY_DELAY}s" >&2
