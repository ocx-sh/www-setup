...
- **pwsh** (`src/install.ps1`) — Windows PS 5.1 Desktop floor + Core. Switch params `-Version -NoModifyPath -Quiet -Force -PrintPath -NoSetup -NoSmoketest`. Env wins over switches. `.zip` archive, zip-slip-safe extraction.
